...
const addReview = async( req, res) => {
    ...
    const reviewtoRestaurant = ((findRestaurant.totlaReview * findRestaurant.rating ) + req?.body?.Rating)/(findRestaurant.totlaReview +1)
    const updateProduct = await Product.findByIdAndUpdate(findOrder?.products?.product,{
     ...
     const updateRestaurant = await Restaurant.findByIdAndUpdate(findOrder?.restaurant,{
